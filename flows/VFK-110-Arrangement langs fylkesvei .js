...
module.exports = {
  ...
  syncPrivatePersonInnsender: {
    ...
    options: {
      condition: (flowStatus) => { // use this if you only need to archive some of the forms.
        return flowStatus.parseJson.result.DialogueInstance.Arrangement_langs_fylkes.Jeg_soker_pa_vegne_av === 'Privatperson' || flowStatus.parseJson.result.DialogueInstance.Arrangement_langs_fylkes.Jeg_soker_pa_vegne_av === 'Lag/forening uten organisasjonsnummer'
      },
  ...
  syncEnterprise: {
    ...
    options: {
      condition: (flowStatus) => { // use this if you only need to archive some of the forms.
        return flowStatus.parseJson.result.DialogueInstance.Arrangement_langs_fylkes.Jeg_soker_pa_vegne_av === 'Organisasjon' || flowStatus.parseJson.result.DialogueInstance.Arrangement_langs_fylkes.Jeg_soker_pa_vegne_av === 'Lag/forening med organisasjonsnummer'
      },
      mapper: (flowStatus) => { // for å opprette person basert på fødselsnummer
        let orgnr
        ...
        } else { throw new Error('Ugyldig tilstand i syncEnterprise') }
        return {
          orgnr
        }
      }
        ...
        return [
          {
            ...
            fields: {
              ...
              Organisasjon: jsonData.Organisasjon3.Organisasjon5.Organisasjonsna || jsonData.Lag_forening2.Organisasjon4.Organisasjon4_orgnavn || jsonData.Lag_forening3.Navn_på_lag_for || 'Ingen organisasjon',
