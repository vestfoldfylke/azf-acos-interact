const retryList = process.env.RETRY_INTERVALS_MINUTES?.split(",").map((numStr) => Number(numStr)) || [15, 60, 240, 3600]
retryList.unshift(0)
...
module.exports = {
  ...
  archiveFileFormats: (process.env.ARCHIVE_FILE_FORMATS?.split(",") || ["JPG", "JPEG", "XLSX", "XLS", "RTF", "MSG", "PPT", "PPTX", "DOCX", "DOC", "PNG"]).map((ext) => ext.toLowerCase()),
  appReg: {
