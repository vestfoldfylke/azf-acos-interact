...
const runCustomJobs = async (flowDef, flowStatus, jobName) => {
  ...
  for (const jobName of customJobs) { // resten håndteres likt som en predefinert jobb
    if (shouldRun(flowDef, flowStatus, jobName)) {
      ...
      try {
        flowStatus[jobName].result = await flowDef[jobName].customJob(flowStatus) // men her trigger vi customJob fra flowDef istedenfor en job fra jobs-mappa. CustomJob blir da typisk steg i flowen som kun trengs i noen få skjemaflyter
  ...
  */
...

  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await parseXml(blobs, flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await syncElevmappe(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await syncEnterprise(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    for (const jobName of syncPrivatePersonJobs) {
      if (shouldRun(flowDef, flowStatus, jobName)) {
        if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
        try {
          flowStatus[jobName].result = await syncPrivatePerson(flowDef[jobName], flowStatus)
          flowStatus[jobName].jobFinished = true
        } catch (error) {
          flowStatus.failed = true
          handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await syncEmployee(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await handleProject(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await handleCase(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await archive(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await signOff(flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await closeCase(flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await sharepointGetListItem(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await sharepointList(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await statistics(flowDef[jobName], flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await groundControl(flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
        handleFailedJob(jobName, flowStatus, error)
  ...
  }
  {
    ...
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        flowStatus[jobName].result = await finishFlow(flowDef.config, flowStatus)
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
