...
const shouldRun = (flowDef, flowStatus, jobName) => {
  ...
  const runAfterTimestampFunction = flowDef[jobName]?.runAfterTimestamp
...
const runCustomJobs = async (flowDef, flowStatus, jobName) => {
  ...
  for (const customJobName of customJobs) { // resten håndteres likt som en predefinert jobb
    if (shouldRun(flowDef, flowStatus, customJobName)) {
      ...
      try {
        const result = await flowDef[customJobName].customJob(flowStatus) // men her trigger vi customJob fra flowDef istedenfor en job fra jobs-mappa. CustomJob blir da typisk steg i flowen som kun trengs i noen få skjemaflyter
...
module.exports = async (flowDef, flowStatus, blobs) => {
  ...
  */
  ...
  {
    const jobName = 'parseJson'
    if (shouldRun(flowDef, flowStatus, jobName)) {
      if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
      try {
        const result = await parseJson(blobs, flowStatus, flowDef[jobName])
        flowStatus[jobName].result = result
        flowStatus[jobName].jobFinished = true
      } catch (error) {
        flowStatus.failed = true
  ...
  {
    const jobNamePrefix = 'sharepointGetListItem'
    const sharepointGetListItemJobs = Object.keys(flowDef).filter(prop => prop.startsWith(jobNamePrefix))
    for (const jobName of sharepointGetListItemJobs) {
      if (shouldRun(flowDef, flowStatus, jobName)) {
        if (!flowStatus[jobName]) flowStatus[jobName] = { jobFinished: false }
        try {
