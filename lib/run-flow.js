...
module.exports = async (flowDef, flowStatus, blobs) => {
  ...
  */
  ...
  const setupAndRunJob = async (jobName, jobFunc, multipleJob = false) => {
    if (!multipleJob) {
    ...
    for (const multipleJobName of multipleJobNames) {
      await runJob(multipleJobName, jobFunc)
    }
  }
  ...
  await setupAndRunJob('parseXml', async () => parseXml(blobs, flowStatus))
  await setupAndRunJob('parseJson', async () => parseJson(blobs, flowStatus, flowDef.parseJson))
  await setupAndRunJob('syncElevmappe', async () => syncElevmappe(flowDef.syncElevmappe, flowStatus))
  await setupAndRunJob('syncEnterprise', async () => syncEnterprise(flowDef.syncEnterprise, flowStatus))
  await setupAndRunJob('syncPrivatePerson', async () => syncPrivatePerson(flowDef.syncPrivatePerson, flowStatus), true)
  await setupAndRunJob('syncEmployee', async () => syncEmployee(flowDef.syncEmployee, flowStatus))
  await setupAndRunJob('handleProject', async () => handleProject(flowDef.handleProject, flowStatus))
  await setupAndRunJob('handleCase', async () => handleCase(flowDef.handleCase, flowStatus))
  await setupAndRunJob('archive', async () => archive(flowDef.archive, flowStatus))
  await setupAndRunJob('signOff', async () => signOff(flowStatus))
  await setupAndRunJob('closeCase', async () => closeCase(flowStatus))
  await setupAndRunJob('sharepointGetListItem', async () => sharepointGetListItem(flowDef.sharepointGetListItem, flowStatus), true)
  await setupAndRunJob('sharepointList', async () => sharepointList(flowDef.sharepointList, flowStatus))
  await setupAndRunJob('statistics', async () => statistics(flowDef.statistics, flowStatus))
  await setupAndRunJob('groundControl', async () => groundControl(flowStatus))
  await setupAndRunJob('failOnPurpose', async () => { throw new Error('Æ feeijla med vilje!') })
  await setupAndRunJob('finishFlow', async () => finishFlow(flowDef.config, flowStatus))
