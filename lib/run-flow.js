...
const handleFailedJob = async (jobName, flowStatus, error) => {
  ...
  if (flowStatus.runs >= retryIntervalMinutes.length) {
    try {
      logger.error('Blob needs care and love. Failed in job {JobName}. Runs: {Runs}/{MaxRuns}. Will not run again. Reset flowStatus.runs to try again. Error: {@Error}', jobName, flowStatus.runs, retryIntervalMinutes.length, errorMsg)
  ...
  try {
    logger.warn('Failed in job {JobName}. Runs: {Runs}/{MaxRuns}. Will retry in {MinutesToWait} minutes. Error: {Error}', jobName, flowStatus.runs, retryIntervalMinutes.length, minutesToWait, errorMsg)
    await save(`${flowStatus.blobDir}/${flowStatus.refId}-flow-status.json`, JSON.stringify(flowStatus, null, 2), blobOptions)
  } catch (error) {
    logger.errorException(error, 'Dritt og møkk... vi fikk ikke lagret flowStatus til bloben. Ting vil potensielt bli kjørt dobbelt opp. Jobben den stoppet på: {JobName}', jobName)
  }
