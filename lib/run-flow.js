...
const handleFailedJob = async (jobName, flowStatus, error) => {
  ...
  if (flowStatus.runs >= retryIntervalMinutes.length) {
    try {
      logger.error(
        "Blob needs care and love. Failed in job {JobName}. Runs: {Runs}/{MaxRuns}. Will not run again. Reset flowStatus.runs to try again. Error: {@Error}",
        jobName,
        flowStatus.runs,
        retryIntervalMinutes.length,
        errorMsg
      )
      await save(`${flowStatus.blobDir}/${flowStatus.refId}-flow-status.json`, JSON.stringify(flowStatus, null, 2), blobOptions)
      await save(
        `${flowStatus.blobDir}/${flowStatus.refId}-${willNotRunAgainFilename}.json`,
        JSON.stringify({ message: "Fiks flow-status.json og slett denne filen om du vil at dette skjemaet skal kjøres igjen" }, null, 2),
        blobOptions
      )
    } catch (error) {
  ...
  try {
    logger.warn(
      "Failed in job {JobName}. Runs: {Runs}/{MaxRuns}. Will retry in {MinutesToWait} minutes. Error: {@Error}",
      jobName,
      flowStatus.runs,
      retryIntervalMinutes.length,
      minutesToWait,
...
const checkCondition = (jobDef, flowStatus, jobName) => {
  ...
  logger.info("Running condition function for {JobName}. Result: {Result}", jobName, runJob)
  return runJob
}
  ...
  if (runAfterTimestampFunction) {
    if (typeof runAfterTimestampFunction === "function") {
      ...
      if (now < new Date(runAfterTimestamp)) {
        ...
        save(`${flowStatus.blobDir}/${flowStatus.refId}-flow-status.json`, JSON.stringify(flowStatus, null, 2), blobOptions).catch((error) =>
          logger.errorException(error, "Failed to save flowStatus blob with refId: {RefId}. Scheisse, ting kan potensielt bli kjørt flere ganger", flowStatus.refId)
        )

        save(
          `${flowStatus.blobDir}/${flowStatus.refId}-run-after-timestamp-${runAfterTimestamp.replaceAll(":", "__")}.json`,
          JSON.stringify({ message: "Denne filen vil ikke bli plukket opp og kjørt før runAfterTimestamp er passert" }, null, 2),
          blobOptions
        ).catch((error) => logger.errorException(error, "Failed to save runAfterTimestamp blob with refId: {RefId}", flowStatus.refId))

        logger.info(
          "Job {JobName} is not ready to run yet. Waiting until {RunAfterTimestamp}. AcosId: {AcosId}, AcosName: {AcosName}, refId: {RefId}",
          jobName,
          runAfterTimestamp,
          flowStatus.acosId,
          flowStatus.acosName,
...
const runCustomJobs = async (flowDef, flowStatus, jobName) => {
  const customJobs = Object.entries(flowDef)
    .filter((entry) => entry[0].startsWith("customJob") && entry[1].runAfter === jobName)
    .map((entry) => entry[0]) // henter aller customJobs som har runAfter lik jobName
  for (const customJobName of customJobs) {
    // resten håndteres likt som en predefinert jobb
...
module.exports = async (flowDef, flowStatus, blobs) => {
  ...
  */
  ...
  await setupAndCheckJob("failOnPurpose", async () => {
