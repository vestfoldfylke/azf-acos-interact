  ...
  */
  ...
  const setupAndRunJob = async (jobName, jobFunc, multipleJob = false) => {
    ...
    for (const multipleJobName of multipleJobNames) {
      ...
      await runJob(multipleJobName, jobFunc(jobDef))
    }
  }
  ...
  await setupAndRunJob('syncPrivatePerson', async (jobDef) => syncPrivatePerson(jobDef, flowStatus), true)
  ...
  await setupAndRunJob('sharepointGetListItem', async (jobDef) => sharepointGetListItem(jobDef, flowStatus), true)
