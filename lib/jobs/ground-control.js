...
module.exports = async (flowStatus) => {
  ...
  if (!flowStatus.parseXml?.jobFinished && !flowStatus.parseJson?.jobFinished)
    throw new Error("Could not find flowStatus.parseXml.jobFinished or flowStatus.parseJson.jobFinished. Did you remember to enable the parseXml or parseJson job for the flow?")
  ...
  for (const file of schemaFilesToCopy) {
    ...
    const sasUrl = await sourceBlob.generateSasUrl({ permissions: BlobSASPermissions.parse("r"), expiresOn: new Date(Date.now() + 60000) })
