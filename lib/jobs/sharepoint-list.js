...
module.exports = async (jobDef, flowStatus) => {
  ...
  for (const fileUpload of fileUploads) {
    try {
      logger.info('sharePointList: Starting file upload to SharePoint. FileName: {FileName}, FileDesc: {FileDesc}, SiteName: {SiteName}', fileUpload.file.name, fileUpload.file.desc, fileUpload.siteName)
      ...
      logger.info('sharePointList: Successfully uploaded file to SharePoint. FileName: {FileName}, FileDesc: {FileDesc}, LegalFilename: {LegalFilename}, SiteName: {SiteName}', fileUpload.file.name, fileUpload.file.desc, legalFilename, fileUpload.siteName)
    } catch (error) {
      failedFileUploads.push(fileUpload)
      const errorMsg = error.response?.data || error.stack || error.toString()
      logger.warn('sharePointList: Failed when uploading file to SharePoint. FileName: {FileName}, FileDesc: {FileDesc}, SiteName: {SiteName}, Error: {@Error}', fileUpload.file.name, fileUpload.file.desc, fileUpload.siteName, errorMsg)
    }
  }
