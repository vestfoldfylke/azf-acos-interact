...
const { storageAccount, graph, sharepointClient, nodeEnv } = require('../../config')
...
const { getListAndSiteId } = require('../graph-actions')
...
module.exports = async (jobDef, flowStatus) => {
  ...
  if (mapper) {
    ...
    listItems = listItems.map(async (item) => {
      if (nodeEnv === 'production') {
        if (!item.prodListUrl) throw new Error('Aiaia, did you forget to set sharepoint-list prodListUrl??')
        const { siteId, listId, siteName } = await getListAndSiteId(item.prodListUrl)
        item.siteId = siteId
        ...
        const { siteId, listId, siteName } = await getListAndSiteId(item.testListUrl)
        item.siteId = siteId
        item.siteName = siteName
        item.listId = listId
