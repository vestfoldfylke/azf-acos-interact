...
const createBatchRequest = (listItems) => {
  ...
  for (const item of listItems) {
    ...
    // Hvis vi har item.elementId så skal vi kjøre en update av elementet istedenfor å opprette det
    ...
    if (item.elementId) {
      const itemReq = {
        ...
        method: 'PATCH',
        path: item.path,
        url: `/sites/${item.siteId}/lists/${item.listId}/items/${item.elementId}/fields`,
        body: item.fields,
        headers: {
...
const repackBatchRequests = (response, originalRequest, listItems, files) => {
  ...
  const uploadFileItemsWithId = needsUploadFileItems.map(item => { // utvid lista fra linja over til å også inneholde listeelementets interne SharePoint-id. Nå har vi en liste over de listeelementene som skal ha opplasting av fil / vedlegg og som har blitt opprettet i sp-lista
    ...
    return {
      ...item,
      spItemId: correspondingResponseItem?.body?.id || item.elementId // Hvis vi har en body.id, er det kjørt Create, og vi bruker den nylig opprettede id'en. Hvis ikke, har vi kjørt Update, og må bruke elementId (vi får ikke tilbake id'en når vi oppdaterer med graph)
    }
  })
...
module.exports = async (jobDef, flowStatus) => {
  ...
  if (mapper) {
    ...
    for (const item of listItems) {
      if (nodeEnv === 'production') {
        ...
        item.elementId = item.prodElementId || null
      } else {
        ...
        item.elementId = item.testElementId || null
      }
    }
  } else {
  ...
  for (const fileUpload of fileUploads) {
    try {
      ...
      const query = `_api/web/lists(guid'${fileUpload.listId}')/items(${fileUpload.spItemId})/AttachmentFiles/add(FileName='${flowStatus.refId}-${fileUpload.file.desc}')`
