...
module.exports = async (jobDef, flowStatus) => {
  ...
  const forceUpdate = personData.forceUpdate !== false // COOP OBS! skitten måte for å default til true

  logger.info('syncEmployee: Syncing employee')
  const { ssn, ansattnummer } = personData
  if (!ssn && !ansattnummer) {
  ...
  logger.info('syncEmployee: Successfully synced employee. PrivatePerson recNo: {PrivatePersonRecNo}, Enterprise recNo: {EnterpriseRecNo}, Manager recNo: {ManagerRecNo}', data.privatePerson.recno, data.responsibleEnterprise.recno, data.archiveManager.recno)
  return data
}
