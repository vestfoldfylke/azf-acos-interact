...
module.exports = async (blobs, flowStatus) => {
  let result = {}
  {
    ...
    result = await xmlParser.parseStringPromise(data)
    result.xmlFile = xmlBlob
    logger.info('parse-xml: Finished egendefinert avleveringsfil. BlobName: {BlobName}', xmlBlob.name)
  }
  {
    logger.info('parse-xml: Running function. Finding websak_hode.xml or <Skjemanavn>_<lowStatus.refId>.xml')
    ...
    result.files = filteredFiles.map(file => {
    ...
    logger.info('parse-xml: Finished websak_hode.xml or <Skjemanavn><_flowStatus.refId>.xml. BlobName: {BlobName}', websakHodeBlob.name)
  }
