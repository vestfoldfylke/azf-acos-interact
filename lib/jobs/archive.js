...
module.exports = async (jobDef, flowStatus) => {
  ...
  for (const attachment of attachmentList) {
    ...
    if (Array.isArray(attachmentContent))
      throw new Error(`Found more than one blob on path ${attachment.path}, please check blob and parse[xml|json].result.files (sannsynligvis noe rart med et av filnavnene)!!`) // Da har blob client returnert flere en en blob!!
  ...
  logger.info("archive: Successfully archived document {DocumentNumber}", data.DocumentNumber)
  return data
}
