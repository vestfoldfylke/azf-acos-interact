...
const singleGraphRequest = async (resource, options = { method: "get", body: {} }) => {
  ...
  const url = options.isNextLink ? resource : `${baseurl}/${beta ? "beta" : "v1.0"}/${resource}`
...
const getWebUrlParts = (webUrl) => {
  ...
  if (!webUrl.includes("/sites/") || !webUrl.startsWith("https://"))
    throw new Error(`url is not valid: ${webUrl}, must be on format https://{tenant}.sharepoint.com/sites/{sitename}/Lists/{listName}/AllItems.aspx`)
