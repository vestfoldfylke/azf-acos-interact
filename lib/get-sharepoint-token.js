...
module.exports = async (forceNew = false) => {
  ...
  logger.info('getSharePointToken - no valid token in cache, fetching new from Microsof')
  let pfxcert
  if (nodeEnv === 'dev') {
  ...
  const config = {
    ...
    privateKeyPassphrase: sharepointCredentials.pfxPassphrase, // passphrase for private key
