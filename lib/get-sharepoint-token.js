...
const parseAzureKeyVaultPfx = (pfxBase64, passphrase) => {
  ...
  try {
    ...
    p12 = forge.pkcs12.pkcs12FromAsn1(asn, true, passphrase ?? null)
  } catch (err) {
...
module.exports = async (forceNew = false) => {
  ...
  const pfxPassphrase = sharepointCredentials.pfxPassphrase || (nodeEnv === "dev" ? null : null)

  const { certificate, key } = parseAzureKeyVaultPfx(pfxcert, pfxPassphrase)
