...
const {
  storageAccount,
  utenlandsreisende: { regionGroupsPrefix }
} = require("../../config")
...
const addUserToRegionGroup = async (regionGroupId, entraUserId) => {
  try {
    ...
    logger.info("add-to-region-group: Added user as a member to region group. GroupId: {GroupId}, UserId: {UserId}", regionGroupId, entraUserId)
  } catch (error) {
    const errorCode = error.response ? JSON.stringify(error.response.data) : error.message
...
const removeUserFromRegionGroup = async (regionGroupId, entraUserId) => {
  try {
    ...
    logger.info("remove-from-region-group: Removed user from region group. GroupId: {GroupId}, UserId: {UserId}", regionGroupId, entraUserId)
  } catch (error) {
    const errorCode = error.response ? JSON.stringify(error.response.data) : error.message
...
const getEntraUserRegionGroups = async (userPrincipalName) => {
  ...
  if (!Array.isArray(value)) {
    logger.error("get-entra-user-region-groups: Invalid user region groups response from graph. Value: {Value}", value)
...
const getRegionGroups = async (countryCodes) => {
  ...
  countryCodes = [...new Set(countryCodes.map((code) => code.trim().toUpperCase()))]

  if (!Array.isArray(value)) {
    logger.error("region-groups: Invalid region groups response from graph. Value: {Value}", value)
    throw new Error("Invalid response from graph")
  }
  ...
  value.forEach((regionGroup) => {
    ...
    if (invalidCountryCodeFormat.length > 0) {
      logger.warn(
        "region-groups: Invalid region group description. GroupId: {GroupId}, Description: {Description}, Invalid country codes: {InvalidCountryCodes}",
        regionGroup.id,
...
const getOverlappingTimespan = async (schemaId, currentRefId, regionGroups, entraUser) => {
  ...
  for (const flowStatus of flowStatuses) {
    ...
    if (!flowStatusParsed?.parseJson?.result?.mapped) {
      continue
    }
...
module.exports = { addUserToRegionGroup, getEntraUserRegionGroups, getOverlappingRegionGroups, getOverlappingTimespan, getRegionGroups, removeUserFromRegionGroup }
