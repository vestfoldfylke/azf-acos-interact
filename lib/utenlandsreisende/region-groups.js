...
const getRegionGroups = async (countryCodes) => {
  ...
  value.forEach((regionGroup) => {
    ...
    if (invalidCountryCodeFormat.length > 0) {
      logger.warn('region-groups: Invalid region group description. GroupId: {GroupId}, Description: {Description}, Invalid country codes: {InvalidCountryCodes}', regionGroup.id, regionGroup.description, invalidCountryCodeFormat)
      return
    }
