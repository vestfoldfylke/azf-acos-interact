...
const mapFlowFile = (filename) => {
  ...
  if (filenameList[0].length === 3 && !Number.isNaN(filenameList[1])) {
...
const dispatcher = async () => {
  ...
  const schemaNames = readdirSync("./flows")
    .map((filename) => mapFlowFile(filename))
    .filter((flow) => flow.filename.endsWith(".js"))
  const flows = []
  ...
  for (const flowDef of flows) {
    ...
    for (const [refId, blobs] of Object.entries(refIdCollections)) {
      ...
      try {
        ...
        if (!flowStatusBlob) {
          logger.info(
            "Could not find flowStatusBlob. Probably first run. Creating new flow status blob -  AcosId {acosid} - Acos name {acosname} - refId {refid}",
            flowDef.acosId,
            flowDef.acosName,
            refId
          )
          ...
          if (runAfterTimestampBlob) {
            ...
            if (runAfterTimestamp.toString() === "Invalid Date") {
              logger.error(
                "Blob has invalid runAfterTimestamp - blobname {blobname}. acosId {acosId}. acos name {acosname}. refId {refid}",
                runAfterTimestampBlob.name,
                flowDef.acosId,
                flowDef.acosName,
                refId
              )
            ...
            if (now < runAfterTimestamp) {
              logger.info(
                "Blob is not ready to run yet - Waiting until {timestamp}. Blobname {blobname}. acosId {acosId}. acos name {acosname}. refId {refid}",
                runAfterTimestamp.toISOString(),
                runAfterTimestampBlob.name,
                flowDef.acosId,
                flowDef.acosName,
                refId
            ...
            try {
              ...
              logger.errorException(
                error,
                "Failed to remove runAfterTimestamp blob {blobname}. AcosId {acosId}. AcosName {acosname}. refId {refid}",
                runAfterTimestampBlob.name,
                flowDef.acosId,
                flowDef.acosName,
                refId
          ...
          if (flowStatus.runs >= retryIntervalMinutes.length) {
            logger.error(
              "Blob has exceeded maximum number of runs and will not run again. Missing doNotRunAgainBlob {willNotRunAgainFilename} - Runs: {runs}/{interval} - AcosId: {acosId} - Acos name: {acosname} - refId: {refid}",
              willNotRunAgainFilename,
              flowStatus.runs,
      ...
      try {
        ...
        logger.errorException(
          error,
          "There is something wrong! Can you hear me Major Tom? Please wake up and do something! Now! Error running flow for AcosId {acosId} AcosName {acosName} refId {refId}",
          flowDef.acosId,
          flowDef.acosName,
          refId
        )
