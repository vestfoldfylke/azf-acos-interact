;(async () => {
  ...
  const mockJobDef = {
    // Equivalent to the jobdef in the flowfile (e.g. if you're testing syncEnterprise, this is the property syncEnterprise in the flowfile)
    enabled: true,
    options: {
      mapper: (flowStatus) => {
        return {
  ...
  const mockFlowStatus = {
    parseXml: {
      // Simply mocking that parseXml is finished and have data
      result: {
        ArchiveData: {
